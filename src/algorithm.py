...
class tracktor:
     ...
     def plot_boxes(self):
         ...
         for img_path in os.listdir(path_image):
             S=[]   
             B=[]
             for activeTrack in setOfActiveTracks:
                 bprev = activeTrack[-1]
                 ...
                 sigma =0.5
                 if s < 0.5 :
                     setOfActiveTracks.remove(activeTrack)
                     setOfTracks.append(activeTrack)
                 else:
                     B.append(bb)
                     S.append(s)
                 # NMS For B <- NMS(B,S,lambda)
                 # ignoring lines 14 to 17
                 for tk,bk in zip(setOfActiveTracks,B):
                     tk.addBoxToTrack(bk)
                 Dt,s = self.predict_boxes(total_img_path)
                 for i in range(len(Dt)):
                     tk = Track()
                     tk.addBoxToTrack(Dt[i])
                     setOfActiveTracks.append(tk)
         ...
         setOfTracks.append(setOfActiveTracks)
     def algo(self):
         for img_path in os.listdir(path_image):
             total_img_path = os.path.join(path_image , img_path)
             bb,s = self.predict_boxes(total_img_path)
